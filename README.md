...
- 🚀 零配置启动 ：提供 NotionLike （笔记）、 Basic （基础）、 Comment （评论）等多种预设，一键接入。
- ⚡️ 现代化交互 ：内置精心设计的 / 斜杠命令菜单、选中悬浮气泡菜单、Markdown 快捷语法。
- 🎨 极致美学 ：样式与逻辑分离，内置 Notion 、 飞书 等多套高颜值主题，支持 CSS 变量级定制。
- 🧩 全功能插件 ：集成表格、图片上传、代码块高亮、任务列表等 20+ 核心插件，按需加载。
- 💪 Vue 3 深度集成 ：完美契合 Vue 3 响应式系统与 Composition API，TypeScript 类型完备。

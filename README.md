...
**XmEditor 解决了这个问题。** 它不是简单的 Tiptap 封装，而是一套完整的 **编辑器构建套件**：
...
- 📑 **大纲目录** — 根据标题自动生成文档目录，支持滚动高亮与点击跳转
- 🖱️ **拖拽手柄** — 提供类似 Notion 的块级菜单和拖拽排序功能
- 📦 **多场景预设** — `NotionLike` · `Basic` · `Comment`，开箱即用
- 🧱 **22+ 内置扩展** — 加粗、斜体、链接、表格、代码块、图片、表情等
...
XmEditor 采用 **Extension-First** 架构——所有功能都是独立扩展，可以自由组合。
...
| **交互增强** | `Emoji` `TextAlign` `ShortcutKeys` `Placeholder` `DragHandle`              | 编辑体验                                     |
| **菜单系统** | `SlashMenu` `BubbleMenu` `FixedMenu` `Toc`                                 | UI 菜单及文档导航                            |
